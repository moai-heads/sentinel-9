...
(async()=>{
  ...
  await send("Page.navigate",{url:"file://"+path.join(ROOT,"index.html")+"?probe=1&seed=3"});
  await sleep(2000);
  ...
  out.start=await ev("(()=>{const g=window.__game,s=g.state;return{phase:s.phase,wave:s.wave,quota:s.quota,spawned:s.spawned,upgTurret:g.upg.turret,turretOnline:g.upg.turret>=1};})()");
  ...
  // wave clear -> choose immediately (no compile wait)
  out.choose=await ev("(()=>{const s=window.__game.state;return{phase:s.phase,choices:(s.choices||[]).map(c=>c.id)};})()");
  out.chooseHasTurret=await ev("(()=>{const s=window.__game.state;return (s.choices||[]).some(c=>c.id==='turret');})()");
